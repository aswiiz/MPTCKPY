...
from .site_data import (
    DEPARTMENTS, GALLERY_CATEGORIES, UPDATED_SOON,
    RECRUITERS, PLACEMENT_STATS, local_media_images
)
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Departments — rename 'tagline' -> 'desc' so template {{ dept.desc }} works
        ctx['depts'] = [
            {**dept, 'desc': dept.get('tagline', ''), 'url': reverse(dept['route'])}
        ...
        ctx['recruiters'] = RECRUITERS
        ctx['placement_stats'] = PLACEMENT_STATS
        # No fake events/projects — leave empty so template shows nothing
        ctx['events_preview'] = []
        ctx['projects'] = []
        return ctx
